...
# -*- coding: utf-8 -*-
"""
tr_build_factors.py — DLAP-TSE Turkey (BIST) Phase 3
======================================================
Builds FF5 (2x3) and HXZ q-factor benchmarks for Turkey from the raw
characteristics panel, using the SAME winsorized-returns convention as
the TSE pipeline (per-month 1%/99% clip).

Sorts (per month, using raw characteristic values):
  size     = exp(size char)  -> median split
  bm       = raw BE/ME       -> 30/70
  roe      = raw ROE         -> 30/70 (FF5 profitability proxy)
  ag       = raw asset growth-> 30/70 (FF5 investment proxy)
FF5: SMB/HML/RMW/CMA from 2x3 intersections, value-weighted (mc weights).
q  : R_ME (size), R_IA (investment = ag), R_ROE.

Outputs (data_tr/factors_winsorized/):
  factors_2x3.csv  year, month, Mkt_RF, SMB, HML, RMW, CMA
  factors_q.csv    year, month, Mkt_RF, ME, IA, ROE
"""
import csv
import os
...
def load_panel():
    """month (y,m) -> list of dicts with raw chars, ret, mc"""
    rf = {}
    for r in read_csv(DATA / "risk_free_rate.csv"):
        y, m = r["month"].split("-")
        rf[(int(y), int(m))] = float(r["monthly_rate_pct"])
    bym = defaultdict(list)
    for r in read_csv(DATA / "characteristics_panel.csv"):
        try:
...
def vw(rs):
    """value-weighted mean return (excess over rf handled by caller)."""
...
def build():
    ...
    for ym in sorted(bym):
        ...
        bm_cut = split("bm", 0.3)
        bm_cut2 = split("bm", 0.7)
        roe_cut = split("roe", 0.3)
        roe_cut2 = split("roe", 0.7)
        ag_cut = split("ag", 0.3)
        ag_cut2 = split("ag", 0.7)
        ...
        ff5_rows.append({"year": ym[0], "month": ym[1],
                         ...
                         "CMA": f"{CMA:.6f}"})
        # q-factors: R_ME, R_IA (low inv - high inv), R_ROE (high roe - low roe)
        r_me = SMB
